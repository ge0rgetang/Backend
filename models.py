...
class users(Base):
    ...
    firebase_id = Column('firebase_id', String(255), nullable = False)
    device_arn = Column('device_arn', String(255), nullable = False, default=0)
    
    def verify_password(self, password):
        pwhash = bcrypt.hashpw(password.encode('utf-8'), self.u_paswd.encode('utf-8'))
        return self.u_paswd == pwhash   

    def __init__(self, u_email, u_name, u_paswd, u_handle, stipend_points=0, u_description='No description set', personal_points=0, key='default', firebase_id='0',device_arn=0):
        self.u_email=u_email
        self.u_name=u_name
        self.u_paswd=u_paswd
        self.u_key=key
...
class friends(Base):
    ...
    last_chat = Column('last_chat', String(255), nullable=True, default='N/A')
    last_chat_timestamp = Column('last_chat_timestamp', DateTime, nullable=True, default=datetime.date(1901,1,1))
...
class anonForumPosts(Base):
    ...
    a_post_id = Column('post_id', Integer, primary_key=True, nullable=False, autoincrement=True)
    a_post_u_id = Column('post_u_id', Integer, ForeignKey("users.u_id"), nullable=False)
    a_post_long = Column('post_long', Numeric(precision=11, scale=8), nullable=True)
    a_post_lat = Column('post_lat', Numeric(precision=10, scale=8), nullable=True)
    a_post_cont = Column('post_cont', String(255), nullable=False)
    a_original_post_id = Column('original_post_id', Integer, nullable=False, default=0)
    a_date_time = Column('date_time', DateTime, default=datetime.datetime.utcnow)
    a_points_count = Column('points_count', Integer, default=0)
    a_date_time_edited = Column('date_time_edited', DateTime, default=datetime.datetime.utcnow)
    a_reply_count = Column('reply_count', Integer, nullable=False, default = 0)
...
class groupDetails(Base):
    ...
    def __init__(self, group_name, group_handle, group_description, group_city, group_lat, group_long, group_category='N/A', group_searchable = True, group_readable = True, group_on_profile = True, group_invite_only = 'H'):
...
class anonForumPostUpvoted(Base):
    ...
    voter_id = Column('voter_id', Integer, ForeignKey("users.u_id"), nullable=False)
    post_id = Column('post_id', Integer, ForeignKey("anonForumPosts.post_id"), nullable=False)
    points = Column('points', Integer, default=0, nullable=False)
    __table_args__ = (PrimaryKeyConstraint('voter_id','post_id',name='forum_voter_post_pk'),)
...
class notific(Base):
    ...
    date_time = Column('date_time', DateTime, default=datetime.datetime.utcnow)    
    notific_cont = Column('notific_cont', String(255), nullable = False)
    notific_subject = Column('notific_subject', String(64), nullable = False)

    def __init__(self, n_u_id, notific_cont, notific_subject):
        self.n_u_id = n_u_id
        self.notific_cont = notific_cont
        self.notific_subject = notific_subject
    
    def __repr__(self):
        return '<Notification ID %r>' % self.id_notific    
